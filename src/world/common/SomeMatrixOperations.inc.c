...
ApiStatus N(SomeMatrixOperation2)(ScriptInstance* script, s32 isInitialCall) {
    ...
    if (!(model->flags & 0x400)) {
        guTranslateF(&model->transformMatrix.mtx[0], 0.0f, temp_f22, 0.0f);
        N(SomeMatrixOperation)(&mtx, temp_f28, temp_f26, temp_f24);
        guMtxCatF(&mtx.mtx[0], &model->transformMatrix.mtx[0], &model->transformMatrix.mtx[0]);
        guTranslateF(&mtx.mtx[0], 0.0f, -temp_f22, 0.0f);
        guMtxCatF(&mtx.mtx[0], &model->transformMatrix.mtx[0], &model->transformMatrix.mtx[0]);
        model->flags |= 0x1400;
    } else {
        guTranslateF(&mtx.mtx[0], 0.0f, temp_f22, 0.0f);
